...
export function renderScalarField(ctx, values, mesh, drawGrid, width, height, rotLon, rotLat) {
  ctx.clearRect(0, 0, width, height);
  ...
  if (mesh.type === "grid") {
    ...
    if (!mesh._gridCacheScalar || mesh._gridCacheScalar.w !== renderW || mesh._gridCacheScalar.h !== renderH) {
      mesh._gridCacheScalar = { w: renderW, h: renderH, imageData: ctx.createImageData(renderW, renderH) };
    }
    const data = mesh._gridCacheScalar.imageData.data;

    for (let y = 0; y < H; y++) {
      for (let x = 0; x < W; x++) {
        const i = y * W + x;
        const vN = (values[i] - vMin) / vRange;
        const rgb = hslToRgb(36, 0.92, 0.12 + 0.72 * vN);
    ...
    ctx.clearRect(0, 0, width, height);
    ctx.putImageData(mesh._gridCacheScalar.imageData, offsetX, offsetY);
  ...
  function colorFor(idx) {
    const vN = (values[idx] - vMin) / vRange;
    const rgb = hslToRgb(36, 0.92, 0.12 + 0.72 * vN);
    return [rgb.r, rgb.g, rgb.b];
  }
